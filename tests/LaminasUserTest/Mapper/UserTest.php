...
//         $this->setUpAdapter('pgsql');
    ...
    public function testIntegrationFindBy($methode, $args, $expectedParams, $eventListener, $entityEqual)
    {
        ...
        foreach ($this->realAdapter as $dbAdapter) {
            ...
            $this->assertInstanceOf('LaminasUser\Entity\User', $return);
            $this->assertEquals($entityEqual, $return);
        }
    ...
    {
        $this->mapper->setTableName('LaminasUser');
        $this->assertEquals('LaminasUser', $this->mapper->getTableName());
    }

    public function testInsertUpdateDelete()
    {
        $baseEntity = new Entity();
        $baseEntity->setEmail('laminas-user-foo@zend-framework.org');
        $baseEntity->setUsername('laminas-user-foo');
        $baseEntity->setPassword('laminas-user-foo');
    ...
    {
        $user = new Entity();
        $user->setEmail('laminas-user@github.com');
