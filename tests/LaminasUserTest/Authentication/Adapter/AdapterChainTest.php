...
class AdapterChainTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $event = $this->getMock('LaminasUser\Authentication\Adapter\AdapterChainEvent');
    ...
    public function testResetAdapters()
    {
        ...
        for ($i=1; $i<=3; $i++) {
            $storage = $this->getMock('LaminasUser\Authentication\Storage\Db');
            $storage->expects($this->once())
                    ->method('clear');

            $adapter = $this->getMock('LaminasUser\Authentication\Adapter\ChainableAdapter');
    ...
    protected function setUpPrepareForAuthentication()
    {
        ...
        $this->event = $this->getMock('LaminasUser\Authentication\Adapter\AdapterChainEvent');
     ...
     *
     * @covers \LaminasUser\Authentication\Adapter\AdapterChain::prepareForAuthentication
     * @expectedException \LaminasUser\Exception\AuthenticationEventException
     */
